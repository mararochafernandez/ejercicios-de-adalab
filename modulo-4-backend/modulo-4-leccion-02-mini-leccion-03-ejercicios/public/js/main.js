...
document.querySelector('.js-btn-get-users').addEventListener('click', () => {
  const filterByName = document.querySelector('.js-input-filter-by-name');
  const filterByEmail = document.querySelector('.js-input-filter-by-email');

  // create query params
  const queryParams = `?filterByName=${filterByName.value}&filterByEmail=${filterByEmail.value}`;
