...
// Eliminar libros en papel y sin stock
// http://localhost:3000/books

app.delete('/books', (req, res) => {
  const query = db.prepare('DELETE FROM books WHERE ebook = 0 AND stock = 0');
  const result = query.run();

  res.json(result);
});

// Los datos de un libro y la categoría a la que pertenece
// http://localhost:3000/books/1
...
app.get('/books/:id', (req, res) => {
  ...
  // get book
  const queryBooks = db.prepare('SELECT * FROM books WHERE id = ?');
  const book = queryBooks.get(id);

  // get main category
  const queryCategories = db.prepare(
    'SELECT name FROM categories WHERE id = ?'
  );
  const mainCategory = queryCategories.get(book.mainCategoryId);

  book.mainCategoryName = mainCategory.name;

  // get secondary categories
  const querySecondaryCategories = db.prepare(
    'SELECT categoryId FROM secondaryCategories WHERE bookId = ?'
  );
  const secondaryCategories = querySecondaryCategories.all(id);

  if (secondaryCategories.length > 0) {
    let secondaryCategoriesNames = [];
    for (const secondaryCategory of secondaryCategories) {
      secondaryCategoriesNames.push(
        queryCategories.get(secondaryCategory.categoryId)
      );
    }

    book.secondaryCategories = secondaryCategoriesNames;
  }
